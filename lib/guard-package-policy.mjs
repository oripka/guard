...
const normalizeThreatIntelConfig = (config = {}) => {
  ...
  return {
    ...
    promptOnBlock: raw.promptOnBlock !== false && raw.askOnBlock !== false,
  ...
  return {
    ...
    async evaluatePackageFetch(pkg, context = {}) {
      ...
      if (threatDecision?.allowed === false) {
        if (threat.promptOnBlock && typeof confirmBlock === 'function') {
          const userDecision = await confirmBlock(normalizedPkg, threatDecision, context)
          if (userDecision?.action === 'allow') {
            emit('supply_chain.threat_intel.allowed_by_user', {
              ...
              duration: userDecision.duration || 'once',
            })
            return {
              allowed: true,
              reason: 'threat-intelligence-user-allow',
              source: threatDecision.source || '',
              duration: userDecision.duration || 'once',
              originalDecision: threatDecision,
            }
          }
        }
