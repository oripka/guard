...
export const normalizeDecisionResource = (resource = {}, fallback = {}) => {
  ...
  if (kind === 'process') {
    return {
      kind,
      command: String(resource.command || fallback.command || fallback.commandLine || ''),
      executablePath: String(resource.executablePath || fallback.executablePath || ''),
      cwd: String(resource.cwd || fallback.cwd || fallback.projectDir || ''),
      bypassReason: String(resource.bypassReason || fallback.bypassReason || fallback.reason || ''),
    }
  }
  if (kind === 'http') {
    return {
