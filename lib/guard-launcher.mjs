...
const loadProfileConfig = ({ repoRoot, configPath, seen = new Set() }) => {
  ...
  if (seen.has(normalized)) {
    throw new Error(`profile import cycle detected at ${normalized}`)
  }
  seen.add(normalized)
...
const installNativeMonitor = ({ repoRoot, targetDir, force }) => {
  ...
  runSync('/usr/bin/codesign', ['--force', '--deep', '--sign', '-', appPath])
  return { path: appPath, guardPath, eventLogPath }
}
...
const MUTABLE_PROFILE_ARRAY_FIELDS = new Set([
  'network.allowedDomains',
  'network.deniedDomains',
...
const readRawProfile = (configPath) =>
  JSON.parse(fs.readFileSync(configPath, 'utf8'))

const writeRawProfile = (configPath, cfg) => {
  fs.writeFileSync(configPath, `${JSON.stringify(cfg, null, 2)}\n`)
}

const ensureProfileArray = (cfg, field, create) => {
...
const normalizeHttpRuleForProfile = (rule = {}) => {
...
const normalizeProfilePort = (port, label = 'port') => {
...
const normalizeRawTcpRuleForProfile = (rule = {}) => {
  const host = typeof rule.host === 'string' ? rule.host.trim().toLowerCase() : ''
...
const stableProfileJson = (value) => {
  if (Array.isArray(value)) return `[${value.map(stableProfileJson).join(',')}]`
  if (value && typeof value === 'object') {
    return `{${Object.keys(value)
      .sort()
      .map((key) => `${JSON.stringify(key)}:${stableProfileJson(value[key])}`)
      .join(',')}}`
  }
  return JSON.stringify(value)
}

const profileRuleHash = (value) =>
  crypto.createHash('sha256').update(stableProfileJson(value)).digest('hex')

const canonicalRuleValue = (field, value) =>
  field === 'network.httpRules'
    ? normalizeHttpRuleForProfile(value)
    : field === 'network.allowedRawTcp'
      ? normalizeRawTcpRuleForProfile(value)
      : String(value)

const ruleMetadataKey = (field, value) =>
  `${field}:${profileRuleHash(canonicalRuleValue(field, value)).slice(0, 16)}`

const ensureRuleMetadata = (cfg, field, value, now = new Date().toISOString(), source = 'cli') => {
  const canonical = canonicalRuleValue(field, value)
  const valueHash = `sha256:${profileRuleHash(canonical)}`
...
const runProfileTls = ({ cwd, profile, action, json }) => {
  ...
  const cfg = readRawProfile(resolved.config)
  ...
  if (action === 'enable' || action === 'disable') {
    ...
    const next = {
      ...(cfg.network.tlsInspection && typeof cfg.network.tlsInspection === 'object'
        ? cfg.network.tlsInspection
        : {}),
      enabled,
    ...
    changed = stableProfileJson(cfg.network.tlsInspection || {}) !== stableProfileJson(next)
    cfg.network.tlsInspection = next
    if (changed) writeRawProfile(resolved.config, cfg)
  }
...
const runProfileMutateArray = ({ cwd, profile, field, value, action, json }) => {
  ...
  const resolved = resolveMutableProjectProfile({ cwd, profile })
  const cfg = readRawProfile(resolved.config)
  const values = ensureProfileArray(cfg, field, action === 'add')
  ...
  const metadata = changed
    ? action === 'add'
      ? ensureRuleMetadata(cfg, field, value)
      : removeRuleMetadata(cfg, field, value)
    : { metadataKey: ruleMetadataKey(field, value), ruleId: `rule_${ruleMetadataKey(field, value).split(':')[1]}` }
  if (changed) writeRawProfile(resolved.config, cfg)
  emitProfileMutationResult({
    ...
    before,
    after: ensureProfileArray(cfg, field, false) || [],
  }, json)
}

const runProfileMutateHttpRule = ({ cwd, profile, rule, action, json }) => {
  const normalizedRule = normalizeHttpRuleForProfile(rule)
  const resolved = resolveMutableProjectProfile({ cwd, profile })
  const cfg = readRawProfile(resolved.config)
  if (!cfg.network) cfg.network = {}
  if (!Array.isArray(cfg.network.httpRules)) cfg.network.httpRules = []
  const key = stableProfileJson(normalizedRule)
  const before = [...cfg.network.httpRules]
  const existingIndex = cfg.network.httpRules.findIndex(
    (entry) => stableProfileJson(normalizeHttpRuleForProfile(entry)) === key,
  )
  let changed = false

  if (action === 'add') {
    if (existingIndex === -1) {
      cfg.network.httpRules.push(normalizedRule)
      changed = true
    }
  } else if (existingIndex !== -1) {
    cfg.network.httpRules.splice(existingIndex, 1)
    changed = true
  }

  const metadata = changed
    ? action === 'add'
      ? ensureRuleMetadata(cfg, 'network.httpRules', normalizedRule)
      : removeRuleMetadata(cfg, 'network.httpRules', normalizedRule)
    : {
        metadataKey: ruleMetadataKey('network.httpRules', normalizedRule),
        ruleId: `rule_${ruleMetadataKey('network.httpRules', normalizedRule).split(':')[1]}`,
      }
  if (changed) writeRawProfile(resolved.config, cfg)
  emitProfileMutationResult({
    action,
...
const runProfileMutateRawTcpRule = ({ cwd, profile, rule, action, json }) => {
  const normalizedRule = normalizeRawTcpRuleForProfile(rule)
  const resolved = resolveMutableProjectProfile({ cwd, profile })
  const cfg = readRawProfile(resolved.config)
  if (!cfg.network) cfg.network = {}
  ...
  const existingIndex = cfg.network.allowedRawTcp.findIndex(
    (entry) => stableProfileJson(normalizeRawTcpRuleForProfile(entry)) === key,
  )
  let changed = false

  if (action === 'add') {
    if (existingIndex === -1) {
      cfg.network.allowedRawTcp.push(normalizedRule)
      changed = true
    }
  } else if (existingIndex !== -1) {
    cfg.network.allowedRawTcp.splice(existingIndex, 1)
    changed = true
  }

  const metadata = changed
    ? action === 'add'
      ? ensureRuleMetadata(cfg, 'network.allowedRawTcp', normalizedRule)
      : removeRuleMetadata(cfg, 'network.allowedRawTcp', normalizedRule)
    : {
        metadataKey: ruleMetadataKey('network.allowedRawTcp', normalizedRule),
        ruleId: `rule_${ruleMetadataKey('network.allowedRawTcp', normalizedRule).split(':')[1]}`,
      }
  if (changed) writeRawProfile(resolved.config, cfg)
  emitProfileMutationResult({
    action,
