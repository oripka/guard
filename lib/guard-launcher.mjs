...
const getNetworkMode = (cfg) => {
  ...
  if (allowedDomains.length > 0 || httpRules.length > 0) return 'allowlist'
  return 'none'
}
...
const collectProfileDoctor = ({ raw, cfg, audit, configPath, profile, source }) => {
  ...
  if (
    ...
    addFinding(
      ...
      'medium',
      'no-network-policy',
      'Profile has neither networkUnrestricted, allowedDomains, httpRules, nor network.ask enabled.',
    )
  }
