...
const cachedPatternDecisionActive = (cache, host) => {
  for (const [pattern, entry] of cache) {
    if (entry.expiresAt && entry.expiresAt <= Date.now()) {
...
export const createDomainFilter = (network = {}, options = {}) => {
  ...
  return async (host, port) => {
    ...
    if (cachedPatternDecisionActive(allowedPatterns, normalizedHost)) {
      record({ host: normalizedHost, port, allowed: true, reason: 'cached-scope-allow' })
      return true
    }
        ...
        Promise.resolve(options.ask(normalizedHost, port))
          .then((decision) => {
            ...
            const expiresAt = typeof decision === 'object' && decision !== null
              ...
              : 0
            const duration = typeof decision === 'object' && decision !== null
              ? String(decision.duration || 'run').toLowerCase()
              : 'run'
            if (duration !== 'once') {
              const hostPattern = typeof decision === 'object' &&
                typeof decision.hostPattern === 'string'
                ? normalizeHost(decision.hostPattern)
                : ''
              if (allow && hostPattern) {
                allowedPatterns.set(hostPattern, { expiresAt })
              } else if (allow) {
