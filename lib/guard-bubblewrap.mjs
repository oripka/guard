...
export const linuxSandboxBackend = ({ network = {}, networkUnrestricted = false, proxyEnabled = false } = {}) => {
  ...
  if (network.allowLoopbackConnections || (Array.isArray(network.allowLoopbackPorts) && network.allowLoopbackPorts.length > 0)) {
    return 'unsupported-loopback-network'
  }
...
export const assertLinuxBubblewrapSupported = (cfg = {}, { proxyEnabled = false } = {}) => {
  ...
  if (backend === 'unsupported-loopback-network') {
    throw new Error(
      'guard: Linux bubblewrap backend cannot express loopback-only port exceptions yet; use networkUnrestricted for trusted local-dev commands',
    )
  }
  return backend
}
...
export const buildBubblewrapArgs = ({
  ...
  if (backend === 'bubblewrap-deny-network') args.push('--unshare-net')
