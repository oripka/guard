...
const linuxNetworkBackend = (network = {}) =>
  String(process.env.GUARD_LINUX_NETWORK_BACKEND || network.linuxBackend || network.linuxNetworkBackend || 'bubblewrap').toLowerCase()

const linuxProxyCapableBackend = (network = {}) =>
  ['host-proxy', 'policy-helper'].includes(linuxNetworkBackend(network))
...
export const linuxSandboxBackend = ({ network = {}, networkUnrestricted = false, proxyEnabled = false } = {}) => {
  ...
  const hasProxyRules =
    network.ask === true ||
    (Array.isArray(network.allowedDomains) && network.allowedDomains.length > 0) ||
    (Array.isArray(network.httpRules) && network.httpRules.length > 0)
  const hasRawTcpRules = Array.isArray(network.allowedRawTcp) && network.allowedRawTcp.length > 0
  if (linuxProxyCapableBackend(network) && (proxyEnabled || hasProxyRules || hasRawTcpRules)) {
    return 'bubblewrap-host-proxy-network'
  }
...
export const assertLinuxBubblewrapSupported = (cfg = {}, { proxyEnabled = false } = {}) => {
  ...
  if (backend === 'unsupported-proxy-network') {
    throw new Error(
      'guard: Linux bubblewrap backend does not support Guard proxy/domain/httpRules enforcement in rootless mode; set network.linuxBackend=host-proxy or GUARD_LINUX_NETWORK_BACKEND=host-proxy for proxy-aware Linux policy, or use the future policy-helper backend for kernel-enforced fail-closed egress',
    )
  }
