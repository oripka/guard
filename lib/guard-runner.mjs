...
const proxyEnabled =
  ...
  (process.platform === 'linux'
    ? (linuxProxyCapableBackend && (askNetworkEnabled || hasProxyNetworkRules))
...
const main = async () => {
  ...
  if (proxyEnabled && networkBackend === 'iron-proxy') {
    ...
    const httpProxy = await startHttpProxy({ filter, onTraffic: recordNetworkFlow, listenHost })
  ...
  const proxyEnv = proxyEnabled
    ? buildProxyEnv({ httpPort: httpProxyPort, socksPort: socksProxyPort, host: proxyHost })
    : []
