...
const resolveGuarddIronProxyCA = async () => {
  ...
  const ca = await requestGuarddJson('/tls/ca').catch(() => null)
  const activeCa = ca?.lifecycle === 'active' && ca?.generated?.certificate && ca?.generated?.privateKey
    ? ca
    : await requestGuarddJson('/tls/ca', {
        method: 'POST',
        body: { action: 'generate', days: Number(tlsInspection.caDays || 1), commonName: 'Guard iron-proxy CA' },
      })
  ...
  for (const host of ironProxyWarmHosts()) {
    try {
      const result = await requestGuarddJson('/tls/cert', {
        method: 'POST',
        body: { host, days: Number(tlsInspection.leafDays || 1) },
      })
...
const askHttpPolicyInDialog = async (request, suggestedRule) => {
  ...
  const script = [
    `set guardChoice to choose from list {"Deny", "Allow Exact", "Allow Path", "Allow Domain"}`,
    `with prompt "${message}"`,
  ...
  return await new Promise((resolve) => {
    ...
    child.on('exit', (code) => {
      if (code !== 0) {
        resolve({ action: 'deny' })
      } else if (/Allow Domain/.test(stdout)) {
        resolve({ action: 'allow', rule: { host: request.host } })
...
const askHttpPolicyInNative = async (request, suggestedRule) => {
  ...
  if (result.action === 'allow' && result.rule && typeof result.rule === 'object') {
    return { action: 'allow', rule: result.rule }
  }
  return { action: 'deny' }
}
...
const startPolicyDecisionServer = async () => {
  ...
  const decide = async (request) => {
    ...
    if (daemonPolicyEnabled) {
      try {
        ...
        if (result.action === 'allow') {
          runtimeRules.push(suggestedRule)
          recordNetworkDecision({
            host: normalized.host,
            method,
            path: normalized.path,
            ...
            ruleId: result.ruleId || ruleIdFor(suggestedRule),
            suggestedRule,
          })
          return {
            action: 'allow',
    ...
    const ask = () =>
      resolveAskUi() === 'native'
        ? askHttpPolicyInNative(normalized, suggestedRule)
        : isDialogAskUi()
