...
import { startHttpProxy, startSocksProxy, createDomainFilter, buildProxyEnv, createSocketFreePackageLookup, detectPackageFetch } from './guard-network.mjs'
...
import { createPackagePolicy, isPackagePolicyEnabled } from './guard-package-policy.mjs'
import { scanNodeLifecycleScripts } from './guard-install-sandbox.mjs'
import {
  commandFingerprint,
  createTelemetryContext,
  redactProcessArguments,
} from './guard-security-telemetry.mjs'
...
const packagePolicyEnabled = isPackagePolicyEnabled(cfg.supplyChain || {})
const packageLookupConfig = network.packageLookup && typeof network.packageLookup === 'object'
  ? network.packageLookup
  : cfg.supplyChain?.packageLookup && typeof cfg.supplyChain.packageLookup === 'object'
    ? cfg.supplyChain.packageLookup
    : {}
const packageLookupProvider = String(
  process.env.GUARD_PACKAGE_LOOKUP_PROVIDER ||
    packageLookupConfig.provider ||
    '',
).trim().toLowerCase()
const packageLookupEnabled =
  packageLookupConfig.enabled !== false &&
  packageLookupProvider === 'socket-free'
const linuxNetworkBackend = String(process.env.GUARD_LINUX_NETWORK_BACKEND || network.linuxBackend || network.linuxNetworkBackend || 'bubblewrap').toLowerCase()
const linuxProxyCapableBackend = ['host-proxy', 'policy-helper'].includes(linuxNetworkBackend)
const proxyEnabled =
  !cfg.networkUnrestricted &&
  (process.platform === 'linux'
    ? (linuxProxyCapableBackend && (askNetworkEnabled || hasProxyNetworkRules || packagePolicyEnabled || packageLookupEnabled))
    : (askNetworkEnabled || hasProxyNetworkRules || packagePolicyEnabled || packageLookupEnabled))
const runtimeBackend = process.platform === 'linux' ? 'bubblewrap' : 'sandbox-exec'
const linuxPolicyHelperEnabled = process.platform === 'linux' && linuxNetworkBackend === 'policy-helper'
...
const runChecked = (command, args, { message = command } = {}) => {
  const result = spawnSync(command, args, { encoding: 'utf8' })
  if (result.status !== 0) {
    throw new Error(`${message}: ${result.stderr || result.stdout || `exit ${result.status}`}`.trim())
  }
  return result
}
...
const commandSummary = commandArgs
  ...
  .join(' ')
const structuredCommandArguments = redactProcessArguments(commandArgs)
const telemetryContext = createTelemetryContext({
  runId: process.env.GUARD_RUN_ID || '',
  policy: cfg,
})
const guardedCommandFingerprint = commandFingerprint(commandArgs)
...
const resolveExecutablePath = (command = '') => {
  ...
  const pathEntries = (process.env.GUARD_INNER_PATH || process.env.PATH || '').split(path.delimiter).filter(Boolean)
...
const shebangExecutables = (filePath = '') => {
  ...
  try {
    ...
    try {
      ...
      firstLine = buffer.subarray(0, bytes).toString('utf8').split(/\r?\n/, 1)[0]
    } finally {
      fs.closeSync(fd)
    }
  } catch {
  ...
  if (path.basename(interpreter) === 'env') {
    const envCommand = parts.slice(1).find((part) => part && !part.startsWith('-'))
    const resolved = resolvePathCommand(envCommand)
...
const executablePaths = (() => {
  ...
  for (const candidate of [...values]) {
    for (const shebangPath of shebangExecutables(candidate)) {
      if (shebangPath && !values.includes(shebangPath)) values.push(shebangPath)
      if (process.platform === 'darwin' && shebangPath === '/bin/sh' && !values.includes('/bin/bash')) {
        values.push('/bin/bash')
      }
      try {
        const real = fs.realpathSync(shebangPath)
        if (real && !values.includes(real)) values.push(real)
      } catch {}
    }
  }
...
const baseEvent = (type, value = {}) => {
  const launcher = launcherContextPayload()
  const pid = Number(value.pid || activeChildPid || process.pid) || 0
  const parentPid = Number(
    value.parentPid ||
      (activeChildPid && pid === activeChildPid ? process.pid : process.ppid),
  ) || 0
  const event = {
    ...
    processArguments: structuredCommandArguments,
    commandFingerprint: guardedCommandFingerprint,
    pid,
    parentPid,
    responsiblePid: launcher.launcherPid || process.ppid,
    ...launcher,
    ...value,
  }
  return telemetryContext.decorate(event, {
    parentEventId: value.parentEventId === undefined ? activeProcessEventId : value.parentEventId,
  })
}

const recordGuardEvent = (type, value = {}) => {
  const event = baseEvent(type, value)
  appendJsonLine(eventLogPath, event)
  if (type === 'process.started') {
    activeProcessEventId = event.eventId
    telemetryContext.setCausalParent(event.eventId)
  }
  return event
}
...
const recordIronProxyAudit = (auditLine) => {
  ...
  recordNetworkFlow({
    ...
    route: 'proxy',
    tlsInspectionStatus: audit.mode === 'https' || audit.sni
      ? (tlsInspectionEnabled ? 'inspected' : 'tunneled')
      : 'not-applicable',
    host,
    destinationIp: String(audit.destination_ip || audit.upstream_ip || parsed.destination_ip || ''),
    method,
    path: requestPath,
    status: errored ? 'error' : denied ? 'denied' : 'allowed',
    statusCode,
    durationMs: safeNumber(audit.duration_ms || parsed.duration_ms, 0),
    bytesSent: safeNumber(
      audit.bytes_sent ||
        audit.request_bytes ||
        parsed.bytes_sent ||
...
const setupLinuxPolicyNamespace = ({ resolvedRawTcpRules = [] } = {}) => {
  if (!linuxPolicyHelperEnabled) return null
  if (typeof process.getuid === 'function' && process.getuid() !== 0) {
    throw new Error('guard: Linux policy-helper requires root privileges for network namespace and nftables setup')
  }
  for (const command of ['ip', 'nft']) {
    if (!commandExists(command)) {
      throw new Error(`guard: Linux policy-helper requires ${command} on PATH`)
    }
  }

  const suffix = crypto.randomBytes(4).toString('hex')
  const namespace = `guard-${suffix}`
  const hostInterface = `gdh${suffix}`
  const namespaceInterface = `gdn${suffix}`
  const hostAddress = '169.254.77.1'
  const namespaceAddress = '169.254.77.2'
  const subnet = '169.254.77.0/30'

  const cleanup = () => {
    spawnSync('ip', ['netns', 'delete', namespace], { stdio: 'ignore' })
    spawnSync('ip', ['link', 'delete', hostInterface], { stdio: 'ignore' })
  }
  cleanup()

  runChecked('ip', ['netns', 'add', namespace], { message: 'guard: failed to create Linux policy namespace' })
  runChecked('ip', ['link', 'add', hostInterface, 'type', 'veth', 'peer', 'name', namespaceInterface], {
    message: 'guard: failed to create Linux policy veth pair',
  })
  runChecked('ip', ['link', 'set', namespaceInterface, 'netns', namespace], {
    message: 'guard: failed to move policy veth into namespace',
  })
  runChecked('ip', ['addr', 'add', `${hostAddress}/30`, 'dev', hostInterface], {
    message: 'guard: failed to assign host policy address',
  })
  runChecked('ip', ['link', 'set', hostInterface, 'up'], {
    message: 'guard: failed to bring up host policy interface',
  })
  runChecked('ip', ['netns', 'exec', namespace, 'ip', 'addr', 'add', `${namespaceAddress}/30`, 'dev', namespaceInterface], {
    message: 'guard: failed to assign namespace policy address',
  })
  runChecked('ip', ['netns', 'exec', namespace, 'ip', 'link', 'set', namespaceInterface, 'up'], {
    message: 'guard: failed to bring up namespace policy interface',
  })
  runChecked('ip', ['netns', 'exec', namespace, 'ip', 'link', 'set', 'lo', 'up'], {
    message: 'guard: failed to bring up namespace loopback',
  })

  const nftLines = [
    'flush ruleset',
    'table inet guard_policy {',
    '  counter denied_output {}',
    '  chain output {',
    '    type filter hook output priority 0; policy drop;',
    '    oifname "lo" accept',
    '    ct state established,related accept',
    `    ip daddr ${hostAddress} tcp dport { __GUARD_PROXY_PORTS__ } accept`,
  ]
  for (const rule of resolvedRawTcpRules) {
    const ipFamily = net.isIP(rule.ip)
    if (ipFamily === 4) {
      nftLines.push(`    ip daddr ${rule.ip} tcp dport ${rule.port} accept`)
    } else if (ipFamily === 6) {
      nftLines.push(`    ip6 daddr ${rule.ip} tcp dport ${rule.port} accept`)
    }
  }
  nftLines.push('    counter name denied_output log prefix "guard policy-helper denied output " drop')
  nftLines.push('  }')
  nftLines.push('  chain input {')
  nftLines.push('    type filter hook input priority 0; policy drop;')
  nftLines.push('    iifname "lo" accept')
  nftLines.push('    ct state established,related accept')
  nftLines.push('  }')
  nftLines.push('}')

  recordGuardEvent('linux.policy_helper.namespace_created', {
    backend: 'policy-helper',
    namespace,
    hostInterface,
    namespaceInterface,
    hostAddress,
  ...
  const installFirewall = ({ httpProxyPort, socksProxyPort }) => {
    const proxyPorts = [httpProxyPort, socksProxyPort].filter(Boolean).join(', ')
    const ruleset = nftLines.join('\n').replace('__GUARD_PROXY_PORTS__', proxyPorts || '1')
    const rulesetPath = path.join(process.env.GUARD_RUN_DIR, 'policy-helper.nft')
    fs.writeFileSync(rulesetPath, `${ruleset}\n`)
    runChecked('ip', ['netns', 'exec', namespace, 'nft', '-f', rulesetPath], {
      message: 'guard: failed to install Linux policy-helper nftables rules',
    })
    recordGuardEvent('linux.policy_helper.firewall_installed', {
      backend: 'policy-helper',
      namespace,
      proxyHost: hostAddress,
      httpProxyPort: httpProxyPort || 0,
      socksProxyPort: socksProxyPort || 0,
      rawTcpRules: resolvedRawTcpRules.map((rule) => ({
        ruleId: rule.ruleId,
        host: rule.host || '',
        ip: rule.ip,
        port: rule.port,
  ...
  const recordDenials = () => {
    const result = spawnSync('ip', ['netns', 'exec', namespace, 'nft', '-j', 'list', 'counter', 'inet', 'guard_policy', 'denied_output'], {
      encoding: 'utf8',
    })
    if (result.status !== 0 || !result.stdout) return
    try {
      const parsed = JSON.parse(result.stdout)
      const counter = parsed?.nftables?.find((entry) => entry.counter)?.counter || {}
      const packets = Number(counter.packets || 0)
      const bytes = Number(counter.bytes || 0)
      if (packets > 0) {
        recordGuardEvent('sandbox.denial', {
          backend: 'policy-helper',
          source: 'linux-nftables',
          result: 'deny',
          severity: 'medium',
          category: 'network',
          operation: 'network-outbound',
          target: 'non-allowlisted direct socket',
          packets,
          bytes,
          ruleTag: runSandboxLogTag,
...
const askNetworkAccessInNative = async (target, host, port) => {
  ...
  const hasHostScope = rule && Object.prototype.hasOwnProperty.call(rule, 'host')
  return {
    action: result.action === 'allow' ? 'allow' : 'deny',
    duration: result.duration || 'run',
    hostPattern: hasHostScope
...
const isLoopbackBindPermissionError = (error) =>
  ['EACCES', 'EPERM'].includes(error?.code) &&
  /(?:127\.0\.0\.1|localhost|listen|bind)/i.test(
    [error?.address, error?.syscall, error?.message].filter(Boolean).join(' '),
  )

const loopbackControlPlaneError = (error, operation) => {
  if (!isLoopbackBindPermissionError(error)) return error
  const wrapped = new Error(
    [
      `guard: outer sandbox blocked Guard's loopback control-plane bind while starting ${operation}.`,
      'Allow Guard itself to bind and accept localhost high ports, then let Guard enforce the child process policy.',
      'For Codex-style nested sandboxes, the outer sandbox must permit local 127.0.0.1/localhost bind + inbound for Guard proxy and decision listeners.',
...
const getFreeLoopbackPort = async () =>
  await new Promise((resolve, reject) => {
    ...
    server.once('error', (error) => reject(loopbackControlPlaneError(error, 'port allocation')))
...
const waitForTcpPort = async (host, port, timeoutMs = 5000) => {
...
const waitForLoopbackPort = async (port, timeoutMs = 5000) =>
  waitForTcpPort('127.0.0.1', port, timeoutMs)
...
const secretInjectionEnvSourceNames = () => {
  const names = new Set()
  for (const entry of secretInjectionEntries()) {
    const source = entry.source && typeof entry.source === 'object'
      ? entry.source
      : { type: 'env', var: entry.env || entry.var || entry.name }
    const type = String(source.type || 'env').toLowerCase()
    const name = String(source.var || '').trim()
    if (type === 'env' && name) names.add(name)
  }
  return names
}

const omitEnvNames = (env, names) => {
  if (!names || names.size === 0) return env
  const next = { ...env }
...
const buildIronProxyConfig = ({ httpPort, socksPort, caCert, caKey, policyEndpoint, listenHost = '127.0.0.1' }) => {
  ...
  const lines = [
    ...
    '  listen: "off"',
    'proxy:',
    `  http_listen: "${listenHost}:${httpPort}"`,
    '  https_listen: "off"',
    `  tunnel_listen: "${listenHost}:${socksPort}"`,
  ...
  if (askNetworkEnabled || packagePolicyEnabled || packageLookupEnabled) {
...
const packagePromptCache = new Map()

const packagePromptCacheKey = (pkg = {}, decision = {}) =>
  [
    pkg.purl || `${pkg.ecosystem || ''}/${pkg.name || ''}@${pkg.version || ''}`,
    decision.reason || '',
    decision.source || '',
  ].join('|').toLowerCase()

const cachedPackagePromptDecision = (pkg, decision) => {
  const key = packagePromptCacheKey(pkg, decision)
  const cached = packagePromptCache.get(key)
  if (!cached) return null
  if (cached.expiresAt && cached.expiresAt <= Date.now()) {
    packagePromptCache.delete(key)
    return null
  }
  return cached
}

const rememberPackagePromptDecision = (pkg, decision, result = {}) => {
  const duration = result.duration || (result.action === 'allow' ? 'once' : 'run')
  if (duration === 'once') return
  const expiresAt = durationToExpiresAt(duration)
  packagePromptCache.set(packagePromptCacheKey(pkg, decision), {
    action: result.action === 'allow' ? 'allow' : 'deny',
    duration,
    expiresAt,
    createdAt: new Date().toISOString(),
...
const packagePromptSummary = (pkg = {}) =>
  pkg.purl || `${pkg.ecosystem || 'package'}/${pkg.name || 'unknown'}@${pkg.version || '?'}`

const askPackagePolicyInTerminal = async (pkg, decision) => {
  const rl = readline.createInterface({ input: process.stdin, output: process.stderr })
  try {
    const summary = decision.summary || decision.reason || 'Threat intelligence flagged this package'
    const reply = await rl.question(
      `guard: ${packagePromptSummary(pkg)} was flagged: ${summary}. Allow this one download? [y/N] `,
    )
    return {
      action: /^(y|yes)$/i.test(reply.trim()) ? 'allow' : 'deny',
      duration: /^(y|yes)$/i.test(reply.trim()) ? 'once' : 'run',
    }
  } finally {
    rl.close()
  }
...
const askPackagePolicyInDialog = async (pkg, decision) => {
  const message = escapeAppleScriptString([
    `Guard found a known package risk for ${packagePromptSummary(pkg)}.`,
    '',
    decision.summary || decision.reason || 'Threat intelligence flagged this package.',
    decision.source ? `Source: ${decision.source}` : '',
    decision.referenceUrl ? `Reference: ${decision.referenceUrl}` : '',
    '',
    'Deny is the safe default. Allow Once only if you have reviewed this package.',
  ...
  const script = [
    ...
    'default button "Deny"',
    'cancel button "Deny"',
    'with title "Guard Package Warning"',
    'with icon caution',
  ].join(' ')
  ...
  return await new Promise((resolve) => {
    child.on('error', () => resolve({ action: 'deny', duration: 'run' }))
    child.on('exit', (code) => {
      resolve(
        code === 0 && /button returned:Allow Once/.test(stdout)
          ? { action: 'allow', duration: 'once' }
          : { action: 'deny', duration: 'run' },
      )
    })
  })
}

const askPackagePolicyInNative = async (pkg, decision, context = {}) => {
  const result = await runNativeAskHelper('ask-package-policy', {
    package: pkg,
    decision,
  ...
  return {
    action: result.action === 'allow' ? 'allow' : 'deny',
    duration: result.duration || (result.action === 'allow' ? 'once' : 'run'),
  }
}

const confirmPackageBlockDecision = async (pkg, decision, context = {}) => {
  const cached = cachedPackagePromptDecision(pkg, decision)
  if (cached) return cached

  if (!isGraphicalAskUi() && !(process.stdin.isTTY && process.stderr.isTTY)) {
    recordGuardEvent('supply_chain.threat_intel.prompt_skipped', {
      package: pkg,
      reason: 'noninteractive',
      summary: decision.summary || '',
      source: decision.source || '',
    })
    const result = { action: 'deny', duration: 'run', reason: 'noninteractive' }
    rememberPackagePromptDecision(pkg, decision, result)
    return result
  }

  const ask = () =>
    resolveAskUi() === 'native'
      ? askPackagePolicyInNative(pkg, decision, context)
      : isDialogAskUi()
        ? askPackagePolicyInDialog(pkg, decision)
        : askPackagePolicyInTerminal(pkg, decision)
  const result = await ask()
  rememberPackagePromptDecision(pkg, decision, result)
  recordGuardEvent('supply_chain.threat_intel.prompt_decision', {
    package: pkg,
    action: result.action === 'allow' ? 'allow' : 'deny',
...
const startPolicyDecisionServer = async ({ packagePolicy = null, packageFetchLookup = null } = {}) => {
  let runtimeRules = []
  ...
  const decide = async (request) => {
    ...
    const packageFetch = detectPackageFetch({
      host: normalized.host,
      method,
      path: normalized.path,
    })
    if (packageFetch) {
      let packageLookup = null
      if (typeof packageFetchLookup === 'function') {
        packageLookup = await packageFetchLookup(packageFetch)
      }
      recordNetworkFlow({
        phase: 'package-fetch',
        protocol: 'https',
      ...
      if (typeof packagePolicy?.evaluatePackageFetch === 'function') {
        const packageDecision = await packagePolicy.evaluatePackageFetch(packageFetch, {
          packageLookup,
          request: {
            protocol: 'https',
            transport: 'iron-proxy',
            host: normalized.host,
            port: normalized.port || 443,
            method,
            path: normalized.path,
          },
        })
        if (packageDecision?.allowed === false) {
          recordNetworkDecision({
            host: normalized.host,
            method,
            path: normalized.path,
            allowed: false,
            reason: packageDecision.reason || 'package-policy',
            package: packageFetch,
            summary: packageDecision.summary || '',
            referenceUrl: packageDecision.referenceUrl || '',
          })
          return {
            action: 'deny',
            reason: packageDecision.summary || packageDecision.reason || 'package-policy',
          }
        }
      }
    }
    ...
    const askForHttpRule = async ({ upgrade = false } = {}) => {
      ...
      const decideAfterQueuedPrompts = async () => {
        // Requests can arrive concurrently while another prompt is open. Recheck
        // rules inside the queue because the preceding decision may cover this
        // request and may not have existed when decide() first ran.
        runtimeRules = activeRuntimeRules(runtimeRules)
        const queuedTemporaryDecision = matchingTemporaryHttpDecision(normalized)
        if (queuedTemporaryDecision) {
          recordNetworkDecision({
            host: normalized.host,
            method,
            path: normalized.path,
            allowed: queuedTemporaryDecision.action === 'allow',
            reason: 'matched-temporary-http-decision-after-prompt',
            ruleId: queuedTemporaryDecision.id || ruleIdFor(queuedTemporaryDecision.rule || {}),
            rule: queuedTemporaryDecision.rule,
            duration: queuedTemporaryDecision.duration || '',
            expiresAt: queuedTemporaryDecision.expiresAt || '',
          })
          return {
            action: queuedTemporaryDecision.action === 'allow' ? 'allow' : 'deny',
            reason: 'matched-temporary-http-decision-after-prompt',
          }
        }

        const queuedRule = [...configuredRules, ...runtimeRules].find((rule) =>
          ruleMatchesRequest(rule, normalized),
        )
        if (queuedRule) {
          recordNetworkDecision({
            ...
            path: normalized.path,
            allowed: true,
            reason: 'matched-rule-after-prompt',
            ruleId: ruleIdFor(queuedRule),
            rule: queuedRule,
          })
          return { action: 'allow', reason: 'matched-rule-after-prompt' }
        }

        const result = resolveAskUi() === 'native'
          ? await askHttpPolicyInNative(normalized, suggestedRule, { upgrade })
          : isDialogAskUi()
          ? await askHttpPolicyInDialog(normalized, suggestedRule, { upgrade })
          : await askHttpPolicyInTerminal(normalized, suggestedRule, { upgrade })
      ...
      const decision = queue.then(decideAfterQueuedPrompts, decideAfterQueuedPrompts)
      queue = decision.then(() => {}, () => {})
    ...
    if (!askNetworkEnabled) {
      ...
      return { action: 'deny', reason: 'default-deny', suggestedRule }
    }
  ...
  await new Promise((resolve, reject) => {
    server.once('error', (error) => reject(loopbackControlPlaneError(error, 'policy decision server')))
...
const startIronProxy = async ({ packagePolicy = null, packageFetchLookup = null } = {}) => {
  const ironProxy = resolveIronProxy()
  const httpPort = await getFreeLoopbackPort()
  const socksPort = await getFreeLoopbackPort()
  const listenHost = linuxPolicyHelperEnabled
    ? process.env.GUARD_LINUX_POLICY_PROXY_HOST || '127.0.0.1'
    : '127.0.0.1'
  ...
  if (askNetworkEnabled || packagePolicyEnabled || packageLookupEnabled) {
    policyServer = await startPolicyDecisionServer({ packagePolicy, packageFetchLookup })
  ...
  await waitForTcpPort(listenHost, httpPort)
...
const resolvePackageLookup = () => {
  const raw = packageLookupConfig
  const provider = String(
    process.env.GUARD_PACKAGE_LOOKUP_PROVIDER ||
      raw.provider ||
      '',
  ).trim().toLowerCase()
  if (!provider || provider === 'off' || provider === 'none' || raw.enabled === false) {
    return null
  }
  if (provider !== 'socket-free') {
    recordGuardEvent('package.lookup.warning', {
      provider,
      message: 'unknown package lookup provider; package intelligence disabled',
    })
    return null
  }
  ...
  const ttlMs = Math.max(
    1,
    Number.parseInt(String(process.env.GUARD_PACKAGE_LOOKUP_TTL_MS || raw.ttlMs || 60 * 60 * 1000), 10) || 60 * 60 * 1000,
  )
  const timeoutMs = Math.max(
    1,
    Number.parseInt(String(process.env.GUARD_PACKAGE_LOOKUP_TIMEOUT_MS || raw.timeoutMs || 5000), 10) || 5000,
  )
  recordGuardEvent('package.lookup.enabled', {
    provider,
    ttlMs,
    timeoutMs,
    endpoint: 'https://firewall-api.socket.dev/purl/[encodedPurl]',
  })
  return createSocketFreePackageLookup({ ttlMs, timeoutMs })
}

const resolvePackagePolicy = () => {
  if (!packagePolicyEnabled) return null
  const supplyChain = cfg.supplyChain || {}
  recordGuardEvent('supply_chain.package_policy.enabled', {
    threatIntelligence: supplyChain.threatIntelligence?.enabled === true ||
      [
    ...
    dependencyCooldown: supplyChain.dependencyCooldown === true ||
  ...
  return createPackagePolicy({
    supplyChain,
    onEvent: recordGuardEvent,
    confirmBlock: confirmPackageBlockDecision,
  })
}
...
const main = async () => {
  ...
  const packageFetchLookup = resolvePackageLookup()
  const packagePolicy = resolvePackagePolicy()

  let ironProxyCA = ''
  const resolvedRawTcpRules = await resolveAllowedRawTcp(network.allowedRawTcp)
  const linuxPolicyNamespace = setupLinuxPolicyNamespace({ resolvedRawTcpRules })
  const proxyHost = linuxPolicyNamespace?.proxyHost || 'localhost'
  if (linuxPolicyNamespace) {
    process.env.GUARD_LINUX_POLICY_PROXY_HOST = proxyHost
  }

  if (runtimeBackend === 'bubblewrap') {
    assertLinuxBubblewrapSupported(cfg, { proxyEnabled })
    if (proxyEnabled && linuxNetworkBackend === 'host-proxy') {
      recordGuardEvent('linux.network_backend.warning', {
        backend: 'host-proxy',
        enforcement: 'proxy-aware-clients',
        message: 'Linux host-proxy enables Guard proxy, iron-proxy, TLS, and allowedRawTcp policy for proxy-aware workloads, but does not provide kernel-level direct-egress blocking. Use policy-helper when available for fail-closed nft/netns enforcement.',
      })
    }
  }

  if (proxyEnabled && networkBackend === 'iron-proxy') {
    const ironProxy = await startIronProxy({ packagePolicy, packageFetchLookup })
    ...
    const listenHost = linuxPolicyNamespace?.proxyHost || '127.0.0.1'
    const httpProxy = await startHttpProxy({ filter, onTraffic: recordNetworkFlow, packageFetchLookup, packagePolicy, listenHost })
    const socksProxy = await startSocksProxy({ filter, onTraffic: recordNetworkFlow, listenHost })
  ...
  linuxPolicyNamespace?.installFirewall({ httpProxyPort, socksProxyPort })
  ...
  if (cfg.supplyChain?.installSandboxActive?.active === true) {
    recordGuardEvent('supply_chain.install_sandbox', cfg.supplyChain.installSandboxActive)
  }
  ...
  const proxyEnv = proxyEnabled
    ? buildProxyEnv({
        ...
        host: proxyHost,
        caCertPath: ironProxyCA && tlsInspectionEnabled ? ironProxyCA : '',
        caDir: ironProxyCA && tlsInspectionEnabled ? path.dirname(ironProxyCA) : '',
      })
    : []
  const npmUserConfigPath = path.join(process.env.GUARD_TMP_DIR, 'npmrc')
  if (proxyEnabled && httpProxyPort) {
    const proxyValue = `http://${proxyHost}:${httpProxyPort}`
    fs.writeFileSync(npmUserConfigPath, [
      `proxy=${proxyValue}`,
      `http-proxy=${proxyValue}`,
      `https-proxy=${proxyValue}`,
      '',
    ].join('\n'))
  }
  ...
  const secretEnvSourceNames = secretInjectionEnvSourceNames()
  const childProcessEnv = omitEnvNames(process.env, secretEnvSourceNames)
  if (secretEnvSourceNames.size > 0) {
    recordGuardEvent('secret_injection.env_sanitized', {
      sourceType: 'env',
      stripped: [...secretEnvSourceNames].sort(),
    })
  }
  ...
  const runnerEnv = [
    ...
    `NPM_CONFIG_USERCONFIG=${proxyEnabled && httpProxyPort ? npmUserConfigPath : '/dev/null'}`,
    `npm_config_userconfig=${proxyEnabled && httpProxyPort ? npmUserConfigPath : '/dev/null'}`,
  ...
  const childCommand = runtimeBackend === 'bubblewrap'
    ? linuxPolicyNamespace
      ? 'ip'
      : 'bwrap'
  ...
  const childArgs = runtimeBackend === 'bubblewrap'
    ? linuxPolicyNamespace
      ? [
          'netns',
          'exec',
          linuxPolicyNamespace.namespace,
          'bwrap',
          ...buildBubblewrapArgs({
            cfg,
            commandArgs,
  ...
  activeChildPid = child.pid || 0
  recordGuardEvent('process.started', {
    pid: activeChildPid || null,
  ...
  child.on('exit', async (code, signal) => {
    ...
    if (cfg.supplyChain?.lifecycleScriptsActive?.active === true) {
      try {
        const scan = scanNodeLifecycleScripts({
          projectDir: process.env.GUARD_PROJECT_DIR,
          includeNodeModules: cfg.supplyChain.lifecycleScriptsActive.scanNodeModules !== false,
        })
        recordGuardEvent('supply_chain.lifecycle_scripts_scanned', {
          packageManager: cfg.supplyChain.lifecycleScriptsActive.packageManager || '',
          scriptExecution: cfg.supplyChain.lifecycleScriptsActive.scriptExecution || 'unknown',
          scannedManifests: scan.scannedManifests,
          truncated: scan.truncated,
          summary: scan.summary,
          findings: scan.findings.slice(0, 200),
        })
      } catch (error) {
        recordGuardEvent('supply_chain.lifecycle_scripts_scan_failed', {
