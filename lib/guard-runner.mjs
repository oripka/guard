...
const askHttpPolicyInNative = async (request, suggestedRule) => {
  ...
  if (result.action === 'allow' && result.rule && typeof result.rule === 'object') {
    return { action: 'allow', rule: result.rule, duration: result.duration || 'run' }
  }
  return { action: 'deny' }
}
    ...
    if (daemonPolicyEnabled) {
      try {
        ...
        if (result.action === 'allow') {
          runtimeRules.push({
            ...suggestedRule,
            expiresAt: durationToExpiresAt(result.duration),
          })
          recordNetworkDecision({
            host: normalized.host,
            method,
...
const startIronProxy = async () => {
  ...
  child.stderr.on('data', (chunk) => {
    ...
    for (const line of lines) {
      recordIronProxyAudit(line.trim())
    }
  })
  child.stderr.on('close', () => {
    if (stderrBuffer.trim()) {
      recordIronProxyAudit(stderrBuffer.trim())
    }
    stderrBuffer = ''
  })
  cleanups.push(
